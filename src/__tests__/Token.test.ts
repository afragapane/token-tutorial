...
describe('Token', () => {
  ...
  test('has expected properties', async () => {
    await withContracts(async ({ token, accountIDs }) => {
        const [name, symbol, decimals, totalSupply, balance] = await Promise.all([token.name(), token.symbol(), token.decimals(), token.totalSupply(), token.balanceOf(accountIDs[0].address)]);
